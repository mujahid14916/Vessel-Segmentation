...
for i, file in enumerate(drive_test_files, k):
    ...
    image_name = ''.join(file.replace('\\', '/').split('/')[-1].split('.')[:-1])
    shutil.copy(file, input_dir + '/DRIVE_{:02d}.jpg'.format(i))
    label_1 = Image.open(drive_test_label_1_dir + image_name.replace('test', 'manual1') + '.gif')
    label_2 = Image.open(drive_test_label_2_dir + image_name.replace('test', 'manual2') + '.gif')
