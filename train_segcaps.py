...
def main():
    model_list = CapsNetR4(INPUT_SHAPE)
    ...
    log_dir = 'segcaps_logs_full'
    tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)
    mcp_save = tf.keras.callbacks.ModelCheckpoint('models/segcaps-full-model-{epoch:02d}-{loss:.6f}-{out_seg_accuracy:0.6f}.hdf5', monitor='loss', mode='min')
    ...
    history = train_model.fit(full_image_generator('../Corrected', 
                              'pre-processed', 
                              'label', 'png', 
                              batch_size=BATCH_SIZE, 
                              image_size=PATCH_SIZE, caps=True),
