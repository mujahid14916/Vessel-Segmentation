...
def CapsNetR3(input_shape, n_class=2):
    ...
    conv1 = tf.keras.layers.Conv2D(filters=16, kernel_size=5, strides=1, padding='same', activation='relu', name='conv1')(x)
    ...
    # Skip connection
    up_1 = tf.keras.layers.Concatenate(axis=-2, name='up_1')([deconv_cap_1_1, conv_cap_3_1])
    ...
    # Skip connection
    up_2 = tf.keras.layers.Concatenate(axis=-2, name='up_2')([deconv_cap_2_1, conv_cap_2_1])
    ...
    # Skip connection
    up_3 = tf.keras.layers.Concatenate(axis=-2, name='up_3')([deconv_cap_3_1, conv1_reshaped])
    ...
    y = tf.keras.layers.Input(shape=input_shape[:-1]+(1,))
    ...
    def shared_decoder(mask_layer):
        ...
        recon_1 = tf.keras.layers.Conv2D(filters=64, kernel_size=1, padding='same', kernel_initializer='he_normal',
                                activation='relu', name='recon_1')(recon_remove_dim)

        recon_2 = tf.keras.layers.Conv2D(filters=128, kernel_size=1, padding='same', kernel_initializer='he_normal',
                                activation='relu', name='recon_2')(recon_1)

        out_recon = tf.keras.layers.Conv2D(filters=1, kernel_size=1, padding='same', kernel_initializer='he_normal',
                                  activation='sigmoid', name='out_recon')(recon_2)

        return out_recon

    # Models for training and evaluation (prediction)
    train_model = tf.keras.models.Model(inputs=[x, y], outputs=[out_seg, shared_decoder(masked_by_y)])
    # train_model = models.Model(inputs=[x], outputs=[out_seg])
    eval_model = tf.keras.models.Model(inputs=x, outputs=[out_seg, shared_decoder(masked)])

    # manipulate model
    noise = tf.keras.layers.Input(shape=((H, W, C, A)))
    noised_seg_caps = tf.keras.layers.Add()([seg_caps, noise])
    masked_noised_y = Mask()([noised_seg_caps, y])
    manipulate_model = tf.keras.models.Model(inputs=[x, y, noise], outputs=shared_decoder(masked_noised_y))

    return train_model, eval_model, manipulate_model


def CapsNetR4(input_shape, n_class=2):
    x = tf.keras.layers.Input(shape=input_shape)
    ...
    # Layer 3: Convolutional Capsule
    # conv_cap_3_2 = ConvCapsuleLayer(kernel_size=5, num_capsule=8, num_atoms=32, strides=2, padding='same',
    #                                 routings=3, name='conv_cap_3_2')(conv_cap_3_1)
    ...
    # deconv_cap_1_1 = DeconvCapsuleLayer(kernel_size=4, num_capsule=8, num_atoms=32, upsamp_type='deconv',
    #                                     scaling=2, padding='same', routings=3,
    #                                     name='deconv_cap_1_1')(conv_cap_3_2)
    ...
    # Layer 1 Up: Deconvolutional Capsule
    # deconv_cap_1_2 = ConvCapsuleLayer(kernel_size=5, num_capsule=4, num_atoms=32, strides=1,
    #                                   padding='same', routings=3, name='deconv_cap_1_2')(conv_cap_3_1)

    # Layer 2 Up: Deconvolutional Capsule
    # deconv_cap_2_1 = DeconvCapsuleLayer(kernel_size=4, num_capsule=4, num_atoms=16, upsamp_type='deconv',
    #                                     scaling=2, padding='same', routings=3,
    #                                     name='deconv_cap_2_1')(conv_cap_2_2)
    ...
    # Layer 2 Up: Deconvolutional Capsule
    deconv_cap_2_2 = ConvCapsuleLayer(kernel_size=5, num_capsule=4, num_atoms=16, strides=1,
                                      padding='same', routings=3, name='deconv_cap_2_2')(conv_cap_2_1)
...
def CapsNetBasic(input_shape, n_class=2):
    ...
    # Layer 1: Just a conventional Conv2D layer
    conv1 = tf.keras.layers.Conv2D(filters=256, kernel_size=5, strides=1, padding='same', activation='relu', name='conv1')(x)
